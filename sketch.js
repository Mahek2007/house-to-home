...
function draw() {
 ...
 // fill("black"); textSize(25); text(mouseX+","+mouseY,mouseX,mouseY);
  ...
  textStyle(BOLD);
 if(frameCount<100){
text("hint:object clicked after will be overlaping the 1 clicked before if their positions are close or same",200,100);
text("click anywhere on the background to start the game",600,120)
text("click on end button to end the game")
}
...
textSize(70);
fill("red");
text("HAPPY  FATHERS  DAY",150,500);
}
function spawnBed(){
  if (frameCount%60===0){
